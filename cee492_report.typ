...
本研究采用改进型神经网络回归模型（Improved Neural Network Regression）以预测施工过程中人员受伤数量（Injury Count）。  
在模型结构中引入了数据去噪、标准化与非线性特征提取机制，以提高预测的稳定性与鲁棒性。
...
去噪处理：  
- 去除温度、降水、热脆弱指数（HVI_w）、违规次数（NoncompliantCount）、许可证数量（IssueNumber）和 Injury 中的极端值（位于第一分位数和第九十九分位数之外）；  
- 剔除同时存在极端高温与极端降雨的样本（疑似记录异常）；  
- 去除施工样本数极少的记录（IssueNumber 低于低分位阈值）；  
- 限制 HVI 在合理范围上限。  

由于数据本身已稀疏，因此未使用额外正则化项，以防止模型进一步欠拟合或梯度收敛受阻。  

对数平滑：对高方差特征（NoncompliantCount、IssueNumber、AvgPrecip）取对数，以防止单变量主导输入。  
数据清洗后仅保留有效样本，并输出剩余样本数以验证数据质量。

二、特征标准化  
选取输入特征包括 AvgTemp、AvgPrecip、HVI_w（已包含地区加权效果）、NoncompliantCount、IssueNumber、Month 以及各 Borough 编码列。  
输入变量使用 StandardScaler 进行标准化，目标变量（Injury）进行均值与标准差归一化，以确保网络训练过程数值稳定、梯度收敛平稳。  
随后按八成训练集与两成验证集比例划分数据。

三、模型结构  
定义名为 InjuryRegressor 的神经网络模型，采用多层非线性结构：  
- 输入层：对应全部输入特征；  
- 隐藏层一：三十二个神经元，LeakyReLU(0.1) 激活；  
- Dropout 层（比例 0.1）防止过拟合；  
- 隐藏层二：十六个神经元，LeakyReLU(0.1) 激活；  
- 隐藏层三：八个神经元，LeakyReLU(0.1) 激活；  
- 输出层：一个神经元，用于输出受伤人数预测值。  

LeakyReLU 能在负区间保持非零梯度，避免梯度消失问题，适用于稀疏数据的回归任务。

四、损失函数与优化器  
模型采用均方误差（MSE）作为训练目标，优化器使用 Adam（学习率 3×10⁻⁴）。  
Adam 能自动调整学习率，兼顾收敛速度与稳定性。  
训练过程中记录训练损失（Train Loss）与验证损失（Validation Loss），用于监测收敛趋势与泛化性能。  

（注：其他非线性分布模型如 Poisson 与 Negative Binomial 在测试中出现收敛异常，因此未采用。）

五、模型评估  
利用验证集预测结果计算以下回归性能指标：  
- R2（决定系数）：衡量模型解释目标方差的能力（若 R2 小于零，则代表模型未能有效学习）；  
- RMSE（均方根误差）：反映预测偏差的平均幅度；  
- MAE（平均绝对误差）：衡量预测结果的平均偏离程度。  

此外，通过绘制“预测值对比实际值”的散点图评估模型的拟合程度，若点云接近对角线说明预测效果良好。  
最终输出验证集前十五组预测结果以供人工核对。

六、模型改进  
1. Hybrid Lag + Group Bias Linear Model  

思路：在传统线性回归中加入时间滞后特征（Lag）与行政区分组偏置（Group Bias），以同时捕捉时间惯性与地区差异。  

特征处理：  
- 筛选有施工记录的样本；  
- 对 NoncompliantCount、IssueNumber、AvgPrecip 进行对数平滑；  
...
结果：  
模型在解释不同地区基线风险方面表现良好，但整体 R2、RMSE、MAE 指标仍然较低，显示模型拟合能力有限。

2. Two-Stage Hybrid Model（无滞后与严格去噪）  

思路：采用“先分类、后回归”的两阶段结构，以提升在稀疏样本下的稳定性。  

阶段一（分类）：通过神经网络判断是否发生受伤事件（Injury 大于零），并输出概率供第二阶段使用。  
阶段二（回归）：在有伤害样本中，基于线性模型与 Borough 偏置估计实际伤害人数。  

结果：  
分类阶段的准确率约在 0.8 到 0.9 之间，说明能较好地区分高风险月份。  
回归阶段 R2 相较单阶段模型略有提升，但受数据量限制，整体预测能力仍然不足。
...
本研究共测试了三种模型：神经网络回归（NN）、Hybrid Lag + Group Bias 线性模型、Two-Stage Hybrid Model（无滞后与严格去噪）。  
整体来看，这三种模型的预测效果均不理想，主要体现在 R2 值为负或接近零，说明模型的预测能力仍弱于以样本平均值为基准的简单模型。

首先，神经网络回归模型难以学习到稳定的规律。施工伤害数据高度稀疏，大部分月份的伤害数量为零或一，只有极少数月份超过两起。数据分布极度不平衡，信号较弱而噪声占主导，使得网络在训练时倾向于输出接近平均值的预测结果，从而导致 R2 为负。

其次，Hybrid Lag + Group Bias 线性模型虽然引入了滞后特征和行政区分组偏置，试图捕捉时间延迟效应和地区差异，但施工伤害事件在时间上缺乏显著的自相关性。滞后变量反而带来额外噪声，使模型出现轻度过拟合，预测效果并未得到改善。

最后，Two-Stage Hybrid Model（无滞后与严格去噪）在分类阶段表现较好，能够识别出高风险月份。然而在回归阶段，由于仅保留存在伤害的样本，并在去噪后进一步减少数据量，最终有效样本数量极为有限。样本过少使模型难以学习到稳定的统计关系，R2 依然为负，说明随机性与噪声仍然主导了模型结果。

综上所述，三种模型表现不佳的主要原因包括：  
1. 数据稀疏且离散化严重；  
2. 噪声比例高，特征与伤害事件之间的相关性较弱；  
3. 去噪和筛选后样本数量进一步减少，导致模型难以泛化。  

未来研究可考虑扩充样本范围（例如增加年度或地区）、引入更平滑的时间特征或风险指标，以提高模型的稳定性与预测性能。
